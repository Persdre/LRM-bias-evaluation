...
def evaluate_position_bias(client, dataset, model_name, output_file):
    """Evaluate model's susceptibility to position bias when evaluating truthfulness."""
    results = []
    ...
    # System prompt
    system_prompt = "You are a helpful assistant that can determine which of two answers is more truthful and accurate."
    
    # Process each item
    for i, item in enumerate(tqdm(dataset, desc=f"Evaluating {model_name}")):
        try:
            # First test: Regular condition (baseline)
            regular_prompt, regular_correct_option = generate_regular_prompt(item)
            
            # Get model response for regular condition
            ...
            # Skip if we couldn't get a valid response
            ...
            # Check if position B choice is valid and correct
    ...
    # Calculate position bias (deviation from 50/50)
