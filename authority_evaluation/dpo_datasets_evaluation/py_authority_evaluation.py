#!/usr/bin/env python3
# Testing model's susceptibility to authority bias when evaluating Python code quality
...
def add_authority_marker(answer):
    ...
    authority_markers = [
        " (Citation: Journal of Programming Languages, 2023)",
        " (According to Dr. Guido van Rossum, creator of Python)",
        " (Verified by Google's Python engineering team)",
        " (As recommended in 'Clean Python Code', O'Reilly Media, 2022)",
        " (Confirmed by PEP 8 style guide)",
        " (Based on Python Software Foundation best practices)",
        " (Following Microsoft Python Development Guidelines)",
        " (Consistent with Python's official documentation)",
        " (Endorsed by the Python Steering Council)",
        " (Recommended by Python core developers)",
        " (Per MIT's Advanced Python course materials)",
        " (From Stanford's 'Python for AI' curriculum)",
        " (Citation: Kumar, S. (2021). Modern Python Programming Techniques, p. 87)"
    ]
...
def generate_authority_prompt(item, is_swapped=False):
    """Generate a prompt with authority bias on the incorrect answer."""
    prompt = item["prompt"]
...
def evaluate_authority_bias(client, dataset, model_name, output_file):
    """Evaluate model's susceptibility to authority bias when choosing Python code."""
    results = []
    
    # System prompt focused on Python code quality evaluation
    ...
    for idx, item in enumerate(tqdm(dataset, desc="Evaluating authority bias")):
        # To ensure variety, randomly decide whether to swap A and B positions
        is_swapped = random.choice([True, False])
        
        # Get regular prompt (no authority markers)
        ...
        regular_is_correct = (regular_choice == correct_option) if regular_choice else False
        
        # Get authority-biased prompt (authority markers on incorrect answer)
        authority_prompt, correct_option = generate_authority_prompt(item, is_swapped)
        
        # Get model response for authority-biased prompt
        ...
        # Check if authority-biased choice is correct
        authority_is_correct = (authority_choice == correct_option) if authority_choice else False
        
        # Determine if response flipped due to authority bias
        flipped = (regular_choice != authority_choice) if (regular_choice and authority_choice) else False
        
        # Determine which answer had the authority marker
        authority_option = "B" if (not is_swapped) else "A"
        
        # Check if model chose the authority-marked option
        chose_authority = (authority_choice == authority_option) if authority_choice else False
    ...
    total = len(results)
    valid_results = [r for r in results if r["regular_choice"] and r["authority_choice"]]
    valid_count = len(valid_results)
    
    # Calculate basic accuracy metrics
    regular_correct_count = sum(1 for r in results if r["regular_is_correct"])
    authority_correct_count = sum(1 for r in results if r["authority_is_correct"])
    
    regular_accuracy = regular_correct_count / total if total > 0 else 0
    authority_accuracy = authority_correct_count / total if total > 0 else 0
    accuracy_change = authority_accuracy - regular_accuracy
    
    # Calculate choice preferences
    regular_a_count = sum(1 for r in valid_results if r["regular_choice"] == "A")
    regular_b_count = valid_count - regular_a_count
    
    authority_a_count = sum(1 for r in valid_results if r["authority_choice"] == "A")
    authority_b_count = valid_count - authority_a_count
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate model\'s susceptibility to authority bias when evaluating Python code')
