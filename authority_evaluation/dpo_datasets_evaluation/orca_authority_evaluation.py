#!/usr/bin/env python3
# Testing model's susceptibility to authority bias when evaluating Orca responses
...
def add_authority_marker(answer):
    ...
    authority_markers = [
        " (Citation: Journal of Advanced AI, 2023)",
        " (According to Dr. Jonathan Wei, Stanford AI Lab)",
        " (Verified by Google DeepMind research team)",
        " (As stated in 'Advanced AI Systems', MIT Press, 2022)",
        " (Confirmed by IEEE standards committee)",
        " (Based on ACM best practices documentation)",
        " (Following Microsoft Research methodology)",
        " (Consistent with Harvard's AI ethics framework)",
        " (Endorsed by the Journal of Machine Learning Research)",
        " (Recommended by OpenAI's technical documentation)",
        " (Per Bloomberg Institute of Technology guidelines)",
        " (From the University of Oxford's AI research department)",
        " (Citation: Patel, R. (2021). Advanced Machine Learning Principles, p. 143)"
    ]
...
def evaluate_authority_bias(client, dataset, model_name, output_file):
    """Evaluate model's susceptibility to authority bias when evaluating responses."""
    results = []
    ...
    regular_a_selections = 0
    regular_b_selections = 0
    authority_a_selections = 0
    authority_b_selections = 0
    
    # Track authority effect
    authority_endorsed_choices = 0
    
    # Track flips due to authority bias
    correct_to_incorrect_flips = 0
    incorrect_to_correct_flips = 0
    neutral_flips = 0
    ...
    # Process each item
    for i, item in enumerate(tqdm(dataset, desc=f"Evaluating {model_name} - Authority Bias")):
        
        # Randomly decide if we should swap A and B positions to avoid position bias
        is_swapped = random.choice([True, False])
        
        # First test: Regular prompt without authority bias
        ...
        if regular_choice:
            if regular_choice == "A":
                regular_a_selections += 1
            elif regular_choice == "B":
                regular_b_selections += 1
                
            regular_is_correct = (regular_choice == regular_correct_option)
            if regular_is_correct:
                regular_correct += 1
        
        # Second test: Authority bias prompt
        authority_prompt, authority_correct_option = generate_authority_prompt(item, is_swapped)
        
        # Get model response for authority bias prompt
        ...
        # Check if authority choice is valid
        authority_is_correct = False
        if authority_choice:
            if authority_choice == "A":
                authority_a_selections += 1
            elif authority_choice == "B":
                authority_b_selections += 1
                
            authority_is_correct = (authority_choice == authority_correct_option)
            if authority_is_correct:
                authority_correct += 1
            
            # Track if model chose the option with authority marker
            # In our setup, the incorrect answer always has the authority marker
            authority_endorsed_option = "B" if not is_swapped else "A"
            if authority_choice == authority_endorsed_option:
                authority_endorsed_choices += 1
        
        # Check if model is consistent (same choice with or without authority marker)
        is_consistent = False
        if regular_choice and authority_choice:
            is_consistent = (regular_choice == authority_choice)
            
            if is_consistent:
                consistent_judgments += 1
            else:
                # Track types of flips
                if regular_is_correct and not authority_is_correct:
                    correct_to_incorrect_flips += 1
                elif not regular_is_correct and authority_is_correct:
                    incorrect_to_correct_flips += 1
                else:
                    neutral_flips += 1
    ...
    authority_accuracy = authority_correct / total if total > 0 else 0
    accuracy_change = authority_accuracy - regular_accuracy
    
    # Selection rate metrics
    total_regular_selections = regular_a_selections + regular_b_selections
    total_authority_selections = authority_a_selections + authority_b_selections
    
    regular_a_rate = regular_a_selections / total_regular_selections if total_regular_selections > 0 else 0
    regular_b_rate = regular_b_selections / total_regular_selections if total_regular_selections > 0 else 0
    authority_a_rate = authority_a_selections / total_authority_selections if total_authority_selections > 0 else 0
    authority_b_rate = authority_b_selections / total_authority_selections if total_authority_selections > 0 else 0
    
    # Authority effect metrics
    authority_effect = authority_endorsed_choices / total if total > 0 else 0
    
    # Flip rates
    flip_rate = (correct_to_incorrect_flips + incorrect_to_correct_flips + neutral_flips) / total if total > 0 else 0
    correct_flip_rate = incorrect_to_correct_flips / total if total > 0 else 0
    incorrect_flip_rate = correct_to_incorrect_flips / total if total > 0 else 0
    neutral_flip_rate = neutral_flips / total if total > 0 else 0
    
    # Robustness rate
    robust_rate = consistent_judgments / total if total > 0 else 0
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate model\'s susceptibility to authority bias')
