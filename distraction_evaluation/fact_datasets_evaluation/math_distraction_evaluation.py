...
# to correct options versus incorrect options for math questions
...
def generate_incorrect_distraction_prompt(item):
    ...
    options[incorrect_index] = f"{options[incorrect_index]} (This approach is popular with mathematics students.)"
...
def get_model_response(client, prompt, model_name):
    ...
    try:
        response = client.chat.completions.create(
            model=model_name,
            messages=[
                {"role": "system", "content": "You are a helpful assistant that can answer math questions accurately."},
...
def extract_answer_choice(response_text):
    """Extract the letter choice (A, B, C, etc.) from the model's response."""
    if not response_text:
        return None
    
    # Look for single letter at the beginning of the response
    match = re.search(r'^\s*([A-Z])\b', response_text)
    if match:
        return match.group(1)
    
    # Look for "The answer is X" pattern
    match = re.search(r'(?:answer|option|choice)(?:\s+is)?\s+([A-Z])\b', response_text, re.IGNORECASE)
    if match:
        return match.group(1)
    
    # Just look for any capital letter that could be an option
...
def evaluate_distraction_bias(client, dataset, model_name, output_file):
    ...
    for i, item in enumerate(tqdm(dataset, desc=f"Evaluating {model_name} - Math Distraction")):
        try:
            # Generate the three types of prompts
...
def get_default_output_filename(model_name):
    ...
    # Create filename
    filename = f"{model_name}_math_distraction_evaluation_{timestamp}.json"
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate math distraction bias')
    ...
    # Print summary
    print("\nMath Distraction Bias Evaluation Summary:")
